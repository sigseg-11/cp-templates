// https://raw.githubusercontent.com/GuanghaoYe/Top-Trees/master/tree.cc
...
struct SBTree {
    ...
    void update(int now) {
        vmx[now] = max(max(tpt[V[now]].cmx, tpt[V[now]].smx),
                       max(vmx[L[now]], vmx[R[now]]));
        vmn[now] = min(min(tpt[V[now]].cmn, tpt[V[now]].smn),
                       min(vmn[L[now]], vmn[R[now]]));
        sum[now] =
            sum[L[now]] + sum[R[now]] + tpt[V[now]].ssum + tpt[V[now]].csum;
        siz[now] =
            siz[L[now]] + siz[R[now]] + tpt[V[now]].siz + tpt[V[now]].dsiz;
...
bool is_root(int now) {
    return !tpt[now].pnt ||
           (tpt[tpt[now].pnt].ch[0] != now && tpt[tpt[now].pnt].ch[1] != now);
}
void update(int now) {
    tpt[now].cmx = max(tpt[now].val,
                       max(tpt[tpt[now].ch[0]].cmx, tpt[tpt[now].ch[1]].cmx));
    tpt[now].cmn = min(tpt[now].val,
                       min(tpt[tpt[now].ch[1]].cmn, tpt[tpt[now].ch[0]].cmn));
    tpt[now].csum =
        tpt[now].val + tpt[tpt[now].ch[0]].csum + tpt[tpt[now].ch[1]].csum;
    ...
    tpt[now].ssum = SBT.sum[tpt[now].sroot] + tpt[tpt[now].ch[0]].ssum +
                    tpt[tpt[now].ch[1]].ssum;

    tpt[now].siz = SBT.siz[tpt[now].sroot] + tpt[tpt[now].ch[0]].siz +
                   tpt[tpt[now].ch[1]].siz;
...
void make_splus(int now, int v) {
    ...
    tpt[now].smx += v;
    tpt[now].smn += v;
    if (tpt[now].sroot) SBT.Tree_plus(tpt[now].sroot, v);
}
void make_chain(int now, int v) {
    tpt[now].val = v;
    tpt[now].cmake = v;
...
void rotate(int now) {
    ...
    if (!is_root(p)) tpt[anc].ch[tpt[anc].ch[1] == p] = now;
    tpt[now].pnt = anc;
...
int get_prv(int now) {
    ...
    if (tpt[now].ch[0]) {
        ...
        while (now && tpt[tpt[now].pnt].ch[0] == now) now = tpt[now].pnt;
        now = tpt[now].pnt;
        return now;
    }
}
