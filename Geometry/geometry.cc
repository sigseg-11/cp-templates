 ...
 * *, /: scalar multiplication/division (also complex multiplication / division
 * if that's ever useful) abs(p): vector length norm(p): squared vector length
...
#define U(p) \
    ((p) / abs(p))  // unit vector in direction of p (don't use if Z(p) == true)
#define Z(x) (abs(x) < EPS)
#define A(a) \
    (a).begin(), (a).end()  // shortens sort(), upper_bound(), etc. for vectors
...
//"above" here means if l & p are rotated such that l.D points in the +x
//direction, then p is above l
bool above_line(pt p, line l) { return CRS(p - l.P, l.D) > 0; }

// true if p is on line l
bool on_line(pt p, line l) {
    return parallel(l.P - p, l.D) && (!l.S || DOT(l.P - p, l.P + l.D - p) <= 0);
}
...
// intersection pt of l1 and l2, returns INV if they are nonintersecting /
// parallel / identical
...
// p reflected over l
pt refl_pt(pt p, line l) {
    l.S = 0;
    return (ld)2 * cl_pt_on_l(p, l) - p;
}
...
line ang_bis(pt a, pt b, pt c) {
    return line(b, b + INF * (U(a - b) + U(c - b)), 1);
}
...
pt orthocent(pt a, pt b, pt c) {
    return intsct(alt(a, line(b, c, 1)), alt(b, line(a, c, 1)));
}
...
bool dyn_in(pt p, set<pt>& h) {
    ...
    return CRS(*j - p, *i - p) > 0;  // change to >= if border counts as
                                     // "inside"
}

// returns true if p contained in dynamic hull hu / hd
bool dyn_in_hull(pt p, pair<set<pt>, set<pt>>& h) {
    return dyn_in(p, h.first) && dyn_in(-p, h.second);
}
...
void dyn_add_to_hull(pt p, pair<set<pt>, set<pt>>& h) {
    dyn_add(p, h.first), dyn_add(-p, h.second);
}
...
bool in_poly(pt p, vector<pt>& poly) {
    ...
    for (pt q : poly) {
        ...
        lst = q;
        if (on_line(p, s))
            return true;  // change if border not included
        else if (!Z(intsct(l, s) - INV))
            ans = !ans;
    }
    return ans;
}
...
// vector of intersection pts of two circs (up to 2) (if circles same, returns
// empty vector)
