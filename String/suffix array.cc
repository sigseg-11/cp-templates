...
struct RMQ {
    ...
    void build(const vector<T> &values) {
        ...
        for (int k = 0; k < levels; k++) range_min[k].resize(n - (1 << k) + 1);

        if (n > 0) range_min[0] = values;

        for (int k = 1; k < levels; k++)
            for (int i = 0; i <= n - (1 << k); i++)
                range_min[k][i] = better(range_min[k - 1][i],
...
struct suffix_array {
    ...
    void build(const T_string &_str, bool build_rmq = true) {
        ...
        for (int i = 1; i < n; i++)
            rank[suffix[i]] =
                str[suffix[i]] == str[suffix[i - 1]] ? rank[suffix[i - 1]] : i;

        bool done = false;

        for (int len = 1; len < n && !done; len *= 2) {
            // next_index[i] = the next index to use for a suffix of rank i. We
            // insert them in order of the rank of the suffix that comes len
            // characters after the current suffix.
            vector<int> next_index(n);
            iota(next_index.begin(), next_index.end(), 0);

            // Compute the suffix array for 2 * len. Suffixes of length <= len
            ...
            for (int i = 1; i < n; i++) {
                ...
                if (max(s, before) + len < n && rank[s] == rank[before] &&
                    rank[s + len] == rank[before + len]) {
    ...
    void compute_lcp() {
        ...
        for (int i = 0; i < n; i++) {
            ...
            // starting at suffix[r] and at suffix[r - 1]. Note that lcp[0] is
            // always 0.
            lcp[rank[i]] = match;
            match = max(match - 1, 0);
        }
    ...
    int compare(int a, int b, int length = -1) const {
        ...
        if (a + common >= n || b + common >= n) return a + common >= n ? -1 : 1;
